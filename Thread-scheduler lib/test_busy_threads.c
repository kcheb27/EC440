 ...
 * count.
 *
 * Tests failed when value returned by join is wrong. 
 *
...
int main(int argc, char **argv){
  ...
  pthread_create(&thread,NULL, count, (void *)i);
...
int main(int argc, char **argv) {
  ...
  for(i = 0; i < THREAD_CNT; i++) {
    pthread_create(&threads[i + THREAD_CNT], NULL, count, (void *)i+THREAD_CNT);
  }
    for(i = 0; i < THREAD_CNT; i++) {
    void *pret;
    int ret;
    
    pthread_join(threads[i + THREAD_CNT], &pret);
