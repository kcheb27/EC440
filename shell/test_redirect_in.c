...
main(void)
{
  struct pipeline* my_pipeline = pipeline_build("hi < it\n");
  ...
  assert(strcmp("hi", my_pipeline->commands->command_args[0]) == 0);
  assert(my_pipeline->commands->command_args[1] == NULL);
  
  // Test the redirect state
  assert(strcmp("it", my_pipeline->commands->redirect_in_path) == 0);
